...
<script type="text/javascript">
	...
	function initTable() {  
	    ...
	    $("#resourceTable").bootstrapTable({
	        ...
	        url:"../resource/resource!pageCheck.action", 
	        //url:"/STRMS/test.json",
	        striped: true,  //表格显示条纹  
	...
	function operateFormatter(value, row, index) {
	   var s="";
	   s = s + ' <a class="preview">预览</a>';
	   s = s + ' <a class="download">下载</a>';
	   s = s + ' <a class="agreeCheck">审批通过</a>';
	   s = s + ' <a class="disagreeCheck">审批拒绝</a>';
	   return [s].join('');
	}
	...
	window.operateEvents = {
	   ...
	   'click .agreeCheck': function (e, value, row, index) {
		   updateResource(row.resId,10);
	   },
	   'click .disagreeCheck': function (e, value, row, index) {
		   updateResource(row.resId,1);
	   },
	...
	function preview(resId,resType){
		var url = '../resource/resource!downloadFile.action?resource.resId='+resId;
		///resource/resource!d"ownload.action?resource.resId=62
		for(var i=0;i<videoType.length;i++){
			if(resType == videoType[i]){
				$('#videoDiv').modal('show');
				$('#video').attr('src',url);
				return;
			}
